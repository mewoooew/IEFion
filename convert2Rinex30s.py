...
def run_rnx2crx(count, file):
    if file.is_file():
        try:
            sh.rnx2crx(file)
            sh.rm('-f', file)
        except sh.ErrorReturnCode as e:
            print(f'::: {count} converting to CRX exited with {e}')
    else:
        print('WARNING: item must be RNX FILE !')


def rnx2crx_files_in(dirRnx, poolCOUNT=16):
    """p.s. compact rnx files to crx

    Args:
        dirRnx (Path):
    """
    ofiles = sorted(dirRnx.glob('*.*o'))
    for idx, _ in enumerate(ofiles):
        if idx % poolCOUNT == 0:
            pass
        else:
            continue
        ...
        pool = Pool(poolCOUNT)
        arguments = [
            (count, ofile) for count, ofile in enumerate(ofiles[idx : idx + poolCOUNT])
        ]

        results = [
            pool.apply_async(run_rnx2crx, args=argument) for argument in arguments
        ]
        pool.close()
        pool.join()
...
def run_runpkr00(count: int, fT02: Path, dirTGD: Path, INFO: str, stations: str) -> str:
    ...
    fTGD = dirTGD / f'{fT02.stem}.tgd'
    if fTGD.is_file():
        return

    try:
        sh.runpkr00('-d', '-g', fT02, dirTGD)
    except sh.ErrorReturnCode as e:
        print(f'::: {count}th - {obsName}: converting skipped with {e.exit_code}. \n')

    return f'{count}th - {INFO} data processed \n'
...
def convert_TrimbleT02toTGD_runpkr00(
    ...
    filesT02 = sorted(dirT02.glob('*aD.*02'))
    for idx, _ in enumerate(filesT02):
        if idx % poolCOUNT == 0:
            pass
        else:
            continue
        ...
        pool = Pool(poolCOUNT)
        arguments = [
            (count, fT02, dirTGD, INFO, stations)
            for count, fT02 in enumerate(filesT02[idx : idx + poolCOUNT], start=idx)
        ]
        results = [
...
def cal_xyzObs_teqc(
    ...
    for count, fTGD in enumerate(dirTGD.glob('*.tgd'), start=1):
        try:
            sh.teqc(
                '+qc',
                '+dm',
...
def run_teqcConvert(
    ...
    fname = fTGD.stem
    # fNAV2 = Path(dirNAV, f'{fname}.{obsYEAR}n')  # GPS nav file extention: .yrn
    fRNX2 = Path(dirRNX, f'{fname[:7]}0.{obsYEAR}o')
    fCRX2 = Path(dirRNX, f'{fname[:7]}0.{obsYEAR}d')
    if Path.is_file(fRNX2) or Path.is_file(fCRX2):
        return resINFO
...
def convert_TGDtoRNX2_teqc(
    ...
    # mk_archives(dirRNX, ['ofile'])
    # dirRNX = dirRNX / 'ofile'

    filesTGD = sorted(dirTGD.glob('*.tgd'))
    for idx, _ in enumerate(filesTGD):
        if idx % poolCOUNT == 0:
            pass
        else:
            continue
        pool = Pool(poolCOUNT)
        arguments = [
            (count, obsYEAR, obsDOY, fTGD, dirRNX, INFO)
            for count, fTGD in enumerate(filesTGD[idx : idx + poolCOUNT], start=idx)
        ]
        results = [
...
def convert_RNX2toRNX3_gfzrnx(
    ...
    ofiles = sorted(dirRNX.glob(f'*.{obsYEAR}{fsuffix}'))
    ...
    for idx, _ in enumerate(ofiles):
        ...
        pool = Pool(poolCOUNT)
        arguments = [
            (dirRNX, ofile, fUpdate, fTYPE, satellite, count)
            for count, ofile in enumerate(ofiles[idx : idx + poolCOUNT], start=idx)
        ]
        results = [
            pool.apply_async(run_gfzrnx, args=argument) for argument in arguments
        ]
...
def convertT02_to_RINEX(
    ...
    #     pass
    year = 2024
    doys = [doy for doy in range(1, 184)]
    stryear = f'{year}'
    print(
    ...
    for doy in doys:
        ...
        # -------------------------- costs  : 60 min --------------------------------------
        if toRinex2:
            convert_TGDtoRNX2_teqc(stryr, strdoy, dirs['TGD'], dirs['RNX'], poolCOUNT)
        else:
            pass
        ...
        if toCrx:
            rnx2crx_files_in(dirs['RNX'], poolCOUNT=poolCOUNT)
